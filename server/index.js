...
app.get("/sitemap.xml", async (req, res) => {
  try {
    ...
    let urls = `
      <url>
        <loc>https://trendingtopics.space/</loc>
        <lastmod>${new Date().toISOString().split("T")[0]}</lastmod>
        <changefreq>daily</changefreq>
        <priority>1.0</priority>
      </url>
      <url>
        <loc>https://trendingtopics.space/totalblogs</loc>
        <lastmod>${new Date().toISOString().split("T")[0]}</lastmod>
        <changefreq>monthly</changefreq>
        <priority>1</priority>
      </url>
      <url>
        <loc>https://trendingtopics.space/video</loc>
        <lastmod>${new Date().toISOString().split("T")[0]}</lastmod>
        <changefreq>monthly</changefreq>
        <priority>0.6</priority>
      </url>
            <url>
        <loc>https://trendingtopics.space/aboutus</loc>
        <lastmod>${new Date().toISOString().split("T")[0]}</lastmod>
        <changefreq>monthly</changefreq>
        <priority>1</priority>
      </url>
    ...
    blogs.forEach(blog => {
      ...
      urls += `
    <url>
      <loc>https://trendingtopics.space/blog/${blog.slug}</loc>
